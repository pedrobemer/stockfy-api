...
func TestApiFinnhubGetSymbol(t *testing.T) {
	...
	api.Get("/symbol-lookup", finnhub.GetSymbol)

	for _, testCase := range tests {
		jsonResponse := body{}
		resp, _ := MockHttpRequest(app, "GET", "/api/symbol-lookup"+
...
func TestApiGetSymbolPrice(t *testing.T) {
	type body struct {
		Success     bool                `json:"success"`
		Message     string              `json:"message"`
		Error       string              `json:"error"`
	...
	api.Get("/symbol-price", finnhub.GetSymbolPrice)

	for _, testCase := range tests {
		jsonResponse := body{}
		resp, _ := MockHttpRequest(app, "GET", "/api/symbol-price"+
			testCase.pathQuery, testCase.contentType, testCase.idToken, nil)
